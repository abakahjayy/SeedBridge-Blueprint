...
export function FreshRescue() {
  const { data: listings, isLoading } = useListFreshRescue();

  return (
    <div className="flex-1 bg-destructive/5 p-4 md:p-8">
              ...
              <Link key={produce.id} href={`/produce/${produce.id}`}>
                <div className="bg-card border border-destructive/20 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-200 group cursor-pointer">
                  <div className="bg-destructive/10 p-6 flex flex-col items-center border-b relative">
                    ...
                    {produce.hoursToExpiry != null && (
                      <div className="flex items-center gap-1.5 text-destructive font-bold text-sm animate-pulse">
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-baseline gap-2">
                      ...
                      {produce.originalPricePerKg && (
                        <span className="text-sm line-through text-muted-foreground">{formatCurrency(produce.originalPricePerKg)}</span>
                      )}
                    </div>
                  </div>
                </div>
