...
function PreHarvest() {
  ...
  return <div className="flex-1 bg-secondary/5 p-4 md:p-8">
    ...
    const daysUntil = differenceInDays(new Date(produce.harvestDate), /* @__PURE__ */ new Date());
    return <Link key={produce.id} href={`/produce/${produce.id}`}>
                  <div className="bg-card border border-secondary/20 rounded-2xl overflow-hidden hover:shadow-lg transition-all duration-200 group cursor-pointer">
                    ...
                    <div className="p-5">
                      ...
                      {produce.depositRequired && <div className="flex items-center gap-1.5 text-sm font-medium text-secondary bg-secondary/10 rounded-lg px-3 py-1.5">
