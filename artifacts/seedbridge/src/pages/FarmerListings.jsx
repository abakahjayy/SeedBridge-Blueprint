...
function FarmerListings() {
  ...
  const params = { farmerId: user?.id, status: activeTab === "active" ? "available" : void 0 };
  ...
  return <div className="flex-1 bg-muted/30 p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          </div> : data?.items && data.items.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.items.map((produce) => <Card key={produce.id} className="overflow-hidden flex flex-col group">
                <div className="bg-muted/50 p-6 flex justify-center items-center border-b relative">
                  ...
                  {produce.isPreHarvest && <Badge variant="secondary" className="absolute top-3 right-3">Pre-Harvest</Badge>}
                  {produce.freshnessAlert && <Badge variant="destructive" className="absolute top-3 left-3">Rescue Mode</Badge>}
                </div>
