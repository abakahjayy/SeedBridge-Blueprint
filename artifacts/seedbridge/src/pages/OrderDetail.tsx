...
import { ArrowLeft, Package, Truck, CheckCircle2, Clock, ShieldCheck, MapPin } from 'lucide-react';
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-card border rounded-2xl overflow-hidden shadow-sm">
          <div className="bg-primary/5 border-b p-6 flex items-center justify-between">
            ...
            <div className="text-3xl">{CROP_EMOJIS[order.crop] || '📦'}</div>
          </div>
              ...
              {steps.map((step, i) => (
                <div key={step} className="flex flex-col items-center relative z-10 flex-1">
                  <div className={`w-10 h-10 rounded-full border-2 flex items-center justify-center transition-colors ${
                    i <= currentStep ? 'bg-primary border-primary text-primary-foreground' : 'bg-background border-muted-foreground/30 text-muted-foreground'
                  }`}>
                    {i < currentStep ? <CheckCircle2 className="h-5 w-5" /> :
