...
function Marketplace() {
  ...
  const displayProduces = produces.filter((p) => {
    ...
    return !p.isPreHarvest && !p.freshnessAlert;
  });
  return <div className="flex-1 w-full bg-background flex flex-col">
  ...
  >
      ...
      <div className="container mx-auto max-w-6xl px-4 py-8">
        {activeTab === "fresh_rescue" && <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-6 mb-8 flex flex-col md:flex-row items-center gap-6">
        ...
        {isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => <div key={i} className="flex flex-col space-y-3">
          ...
          </div> : displayProduces.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {displayProduces.map((produce) => <ProduceCard key={produce.id} produce={produce} />)}
          </div> : <div className="text-center py-24 bg-card rounded-2xl border border-dashed">
...
function ProduceCard({ produce }) {
  ...
  return <Link href={`/produce/${produce.id}`} className="group h-full">
    ...
    onError={(e) => {
      e.target.src = "https://images.unsplash.com/photo-1596199050105-6d5d32222916?auto=format&fit=crop&q=80&w=600";
    }}
  />
        ...
        <CardContent className="flex-1 p-5 flex flex-col gap-3">
          <div className="flex justify-between items-start">
            <div>
              ...
              {produce.originalPricePerKg && isRescue && <div className="text-sm text-destructive line-through decoration-destructive/50">
          ...
          <div className="space-y-2 mt-2">
            ...
            {isRescue && produce.hoursToExpiry && <div className="flex items-center gap-2 text-sm font-medium text-destructive bg-destructive/10 px-2 py-1 rounded">
