...
import { useCreateProduce } from "@workspace/api-client-react";
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select
    value={formData.crop}
    onValueChange={(val) => setFormData((prev) => ({ ...prev, crop: val }))}
  >
                    <SelectTrigger className="h-12">
                      <SelectValue placeholder="Select crop" />
                    </SelectTrigger>
                    <SelectContent>
                      {Object.entries(CROP_LABELS).map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
