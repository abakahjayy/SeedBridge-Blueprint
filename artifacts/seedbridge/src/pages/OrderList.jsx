...
function OrderList() {
  ...
  const { data, isLoading, refetch } = useListOrders({
    status: activeTab === "all" ? void 0 : activeTab
  });
  ...
  return <div className="flex-1 bg-muted/30 p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        {isLoading ? <div className="space-y-4">
          ...
          </div> : data && data.length > 0 ? <div className="space-y-4">
            {data.map((order) => <Card key={order.id} className="overflow-hidden transition-shadow hover:shadow-md">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-4 mt-auto pt-4 border-t">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium">
                            {order.status === "pending" ? "Awaiting confirmation" : order.status === "in_transit" ? "On the way to destination" : order.status === "delivered" ? "Successfully delivered" : "Order closed"}
                          </span>
                        </div>
                        
                        <div className="flex gap-2">
                          {user?.role === "farmer" && order.status === "pending" && <Button size="sm" onClick={() => handleUpdateStatus(order.id, "confirmed")}>Confirm Order</Button>}
                          {user?.role === "buyer" && order.status === "in_transit" && <Button size="sm" onClick={() => handleUpdateStatus(order.id, "delivered")}>Mark Delivered</Button>}
                          {
