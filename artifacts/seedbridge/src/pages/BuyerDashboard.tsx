                      ...
                      <Link key={order.id} href={`/orders/${order.id}`}>
                        <div className="p-4 hover:bg-muted/50 transition-colors flex items-center justify-between group">
                          ...
                          <div className="text-right">
                            ...
                            <Badge variant={
                              order.status === 'delivered' || order.status === 'completed' ? 'secondary' : 
             ...
             <Card>
                ...
                <CardContent className="p-4">
                  <div className="flex flex-wrap gap-2">
                    {dashboard.favoriteRegions && dashboard.favoriteRegions.length > 0 ? (
                      dashboard.favoriteRegions.map(region => (
                        <Badge key={region} variant="secondary" className="px-3 py-1 font-medium">{region}</Badge>
                      ))
                    ) : (
                      <span className="text-sm text-muted-foreground">No preferred regions set.</span>
                    )}
                  </div>
                </CardContent>
              </Card>
