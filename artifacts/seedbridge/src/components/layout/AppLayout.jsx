...
function AppLayout({ children }) {
  ...
  return <div className="flex min-h-[100dvh] flex-col bg-muted/30">
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navItems.filter((i) => i.show).map((item) => <Link key={item.href} href={item.href} className={cn(
    ...
    /* Mobile bottom nav */
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 border-t bg-background pb-safe z-50">
        <div className="flex items-center justify-around h-16 px-2">
          {navItems.filter((i) => i.show).slice(0, 5).map((item) => <Link key={item.href} href={item.href} className={cn(
